...
def get_salary_aggregate(request, responder):
    ...
    if func_entity:
        ...
        if money_entities:
            ...
            # be returned, convert default (hourly) to that duration.
            # For eg. 'what the average monthly salary'
...
def _resolve_time_in_salary(request, responder, qa):
    ...
    time_entities = [e['value'][0]['value']
                     for e in request.entities
                     if e['type'] == 'sys_time']

    date_compare_ent = [e['value'][0]['cname']
                        for e in request.entities
...
def _get_names(qa_out):
    ...
    names = [str(out['first_name']) + ' ' + str(out['last_name']) +
             ": " + str(out['money']) for out in qa_out]
    names = ', '.join(names)
    return names
