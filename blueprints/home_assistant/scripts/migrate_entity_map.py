...
def migrate_entity_map(app_path, old_entity_map_path):
    ...
    for e in old_entity_map['entities']:
        ...
        entity_data_path = os.path.join(app_path, 'entity-data', '{}-entities.tsv').format(entity_type)
        ...
        map_fields = ['map', 'text-map', 'clause-map']  # fields which existed in an old style entity map
